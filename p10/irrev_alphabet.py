...
k in {25, 50, 67}; the 95% group LCB is one-sided (1.645); reports fixed/paired fibers, retained test mass and groups with no retained event; edges with an endpoint outside the alphabet are dropped from training and test; recompute held-out Delta_inv share and group LCB (alpha 0.5).
...
def run(source, alpha=0.5, ks=(25, 50, 67, 175)):
    ...
    for k in ks:
        sC = sD = sI = 0.0; N = 0; Nall = 0; per = collections.defaultdict(lambda: [0.0, 0]); states_used = set(); fixed = set(); paired = set(); allgroups = set(grp.values())
        for f in range(5):
            ...
            for o in alphabet:
                (fixed if kappa(o) == o else paired).add(o)
        ...
        m = statistics.mean(gi); sd_ = statistics.stdev(gi); lcb = m - 1.645 * sd_ / math.sqrt(len(gi))
        out["k"][k] = dict(target_k=k, states_used=len(states_used), fixed_fibers=len(fixed), paired_fibers=len(paired) // 2, retained_test_mass=round(N / Nall, 4), groups_no_event=len(allgroups) - len(gi), events=N, sigma_C=round(sC / N, 4), sigma_D=round(sD / N, 4), Delta_inv=round(sI / N, 4), share=round((sI / N) / (sC / N), 4), groups=len(gi), group_LCB95=round(lcb, 4))
        print(source, "k", k, out["k"][k], flush=True)
    (H / ("irrev_alphabet_%s_a%g.json" % (source, alpha))).write_text(json.dumps(out, indent=1), encoding="utf-8")


if __name__ == "__main__":
    ap = argparse.ArgumentParser(); ap.add_argument("--source", default="wjazzd"); ap.add_argument("--alpha", type=float, default=0.5); a = ap.parse_args(); run(a.source, a.alpha, ks=(25, 50, 67))
